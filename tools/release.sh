...
#
#   tools/release.sh 1.5.0 [--skip-bench] [--minutes 30]
#
# The release notes are the version's section of CHANGELOG.md, so write
# that entry first; there is no release without one.
#
...
#   5. git push, then the GitHub release with the notes from CHANGELOG.md
set -euo pipefail
cd "$(dirname "$0")/.."
...
# the notes: everything under "## $ver (" up to the next "## ", read now so
# nothing later (the bench's ssh calls, say) can eat them
notes=$(mktemp)
awk -v v="$ver" '/^## /{p = index($0, "## " v " (") == 1; next} p' CHANGELOG.md | sed -e :a -e '/^\n*$/{$d;N;ba' -e '}' > "$notes"
[ -s "$notes" ] || { echo "CHANGELOG.md has no entry for $ver; write one first" >&2; exit 1; }
printf '\nUpdate from Settings, or run shmup_deck.sh from the Scripts menu.\n' >> "$notes"
